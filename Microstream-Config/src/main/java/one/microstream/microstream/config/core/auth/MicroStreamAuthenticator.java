...
public class MicroStreamAuthenticator implements Authenticator<CredentialsUsernamePassword, CredentialsUsernamePassword>
{
	PasswordHasher hashStrategy;

	public MicroStreamAuthenticator(final Class<? extends CredentialsUsernamePassword> authenticationEntityType)
	{
	}
		...
		throws AuthenticationFailedException
	{
		...
		final Optional<User> findFirst = UserDAO.findAll()
			.stream()
			.filter(
				u -> u.getUsername().equals(credentials.username()) && this.hashStrategy.validatePassword(
